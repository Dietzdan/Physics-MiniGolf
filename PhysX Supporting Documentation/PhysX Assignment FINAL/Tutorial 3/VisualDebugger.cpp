...
namespace VisualDebugger
{
	...
	//tests
	PxVec3 BallPos(-5,1,0);
	PxVec3 DirVec(15.f, -1.f, -1.f);
	float Distance;
	float camX;
	float camY;
	float camZ;
	...
	void Init(const char *window_name, int width, int height)
	{
		...
		camera = new Camera(PxVec3(-10.0f, 2.f, 8.0f), PxVec3(15.f,-.1f,-1.f), 5.f);
		...
		//camera->SetPosition(scene->GetSelectedActor()->getGlobalPose().p + BallPos, DirVec);
	...
	void RenderScene()
	{
		...
		int BallSpeed = scene->GetSelectedActor()->getLinearVelocity().x;
		//camera->SetPosition(scene->GetSelectedActor()->getGlobalPose().p + BallPos, DirVec);
		
	
		
			//BallPos = scene->GetSelectedActor()->getGlobalPose().p;
			
			//camera->SetPosition(scene->GetSelectedActor()->getGlobalPose().p + BallPos, PxVec3(15.f, -.1f, -1.f));
	...
	void UserKeyPress(int key)
	{
		...
		case ' ':
			...
			//{
				...
				//ForceSwitch = true;
			//}
			//else if (ForceSwitch == true)
			
				
			
			//Strokes++;
			//scene->ExampleKeyPressHandler();
			//cerr << "Strokes: " << Strokes << endl;
	...
	void KeySpecial(int key, int x, int y)
	{
		...
		case GLUT_KEY_LEFT:
			//BallPos.z -= 1.f;
			Distance = 5.0f;
			camX = Distance * cosf(scene->GetSelectedActor()->getGlobalPose().p.x);
			camZ = Distance * sinf(scene->GetSelectedActor()->getGlobalPose().p.z);
			BallPos -= PxVec3(camX, 0, camZ).cross(PxVec3(0,1,0)).getNormalized();
			break;
		case GLUT_KEY_RIGHT:
			Distance = 5.0f;
			camX = Distance * cosf(scene->GetSelectedActor()->getGlobalPose().p.x);
