...
namespace PhysicsEngine
{
	...
	public:
		...
		// - denisty: 1kg/m^3
		Box(const PxTransform& pose=PxTransform(PxIdentity), PxVec3 dimensions=PxVec3(5.5f,.5f,1.5f), PxReal density=1.f) 
			: DynamicActor(pose)
		{ 
	...
	public:
			...
			:DynamicActor(pose)
		{
			...
			GetShape(0)->setLocalPose(PxTransform(PxVec3(.6f, .3f, 0.f)));
			//GetShape(0)->setLocalPose(PxTransform(PxVec3(0.0f,0.0f,0.0f),PxQuat(1.55f,(PxVec3(0.0f, 1.0f, 0.0f)))));
			GetShape(1)->setLocalPose(PxTransform(PxVec3(0.f,4.5f,0.f)));
	...
	class Flag: public DynamicActor
	{
		//constructor
	public:
		Flag(const PxTransform& pose = PxTransform(PxIdentity), PxVec3 dimensions = PxVec3(.5f, .5f, .5f), PxReal density = 1.f)
			:DynamicActor(pose)
		{
			CreateShape(PxBoxGeometry(dimensions.x, dimensions.y * 8, dimensions.z), density);
			CreateShape(PxBoxGeometry(dimensions.x*4.5f, dimensions.y, dimensions.z * 2), density);

			GetShape(1)->setLocalPose(PxTransform(PxVec3(11.0f, 7.49f, 6.25f), PxQuat(1.57f, (PxVec3(1.0f, 0.0f, 0.0f)))* PxQuat(1.57f, PxVec3(0.0f, .0f, 1.0f))));
			//GetShape(0)->setLocalPose(PxTransform(PxVec3(0.0f,0.0f,0.0f),PxQuat(1.55f,(PxVec3(0.0f, 1.0f, 0.0f)))));
			GetShape(0)->setLocalPose(PxTransform(PxVec3(11.0f, 2.5f, 8.f)));
